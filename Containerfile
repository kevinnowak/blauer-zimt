# Blauwind — minimal bootc image (Milestone 0)
#
...
RUN sed -i \
        -e 's/^NAME=.*/NAME="Blauwind"/' \
        -e 's/^PRETTY_NAME=.*/PRETTY_NAME="Blauwind 44"/' \
        -e 's/^VARIANT=.*/VARIANT="Blauwind"/' \
        -e 's/^VARIANT_ID=.*/VARIANT_ID=blauwind/' \
        /usr/lib/os-release
...
RUN mkdir -p /etc/dnf/libdnf5.conf.d \
    && printf '[main]\ninstall_weak_deps=False\n' > /etc/dnf/libdnf5.conf.d/blauwind.conf
